"""
Main drone controller implementation. Operates drone sortie:

    takeoff -> track faces -> recognize faces -> land

Publishes drone video through message queue for multiple subscibers.
...
class Controller(metaclass=Singleton):
    """Handles drone and CV operations"""

    # Frame dimensions and PID control parameters
    SCALE_FACTOR = 0.5
    WIDTH = SCALE_FACTOR * 960
    HEIGHT = SCALE_FACTOR * 720
    FB_RANGE = (6200, 6800)
    ...
    def track(self, frame):
        ...
        self.drone.send_rc_control(0, fwd, alt, yaw)
    ...
    def start(self, takeoff=False, export=False):
        ...
        try:
            while True:
                ...
                if process_this_frame:
                    # Face recognition
                    face_locations, face_names = self.recognize(small_frame)
                ...
                for (top, right, bottom, left), name in zip(face_locations, face_names):
                    ...
                    cv2.rectangle(frame, (left - 5, top - 5), (right + 5, bottom + 5), colour, 2)

                    # Draw a label with a name below the face
                    cv2.rectangle(frame, (left - 5, bottom - 35), (right + 5, bottom + 5), colour, cv2.FILLED)
                    font = cv2.FONT_HERSHEY_DUPLEX
                    cv2.putText(frame, name, (left + 8, bottom - 8), font, 0.8, text_colour, 1)
...
def main() -> None:
    ...
    controller.start(takeoff=False, export=False)
